...
public class WarehouseInventory extends SinglyListDictionary<String,InventoryRecord> {
		...
		tool util=new tool();
		InventoryRecord[] ArrayIndex;
		BinarySearchTree BSTIndex;
	...
	public InventoryRecord query(String attribute, double perct)
	{
		
		createIndex(attribute);
		int pivot = (int) (ArrayIndex.length * perct);
		if(perct==0)
		{
			return ArrayIndex[0];
		}
	...
	// Create Index for attribute of interest
	public InventoryRecord[] createIndex(String attribute)
	{	
	...
	void command(WarehouseInventory a)
	{
		...
		{	
			...
			case 7:
				...
				util.print(createIndex(ID1)); // Print the sorted list
				break;
			case 8:
				System.out.println("Please type the attributes for the item ");
				util.Traverse(createTreeIndex(scanner_in.next()));
				util.showPath();
				util.showRecord();
				break;
			case 9:
				System.out.println("Please type the attributes for the item ");
				String attribute = scanner_in.next();
				System.out.println("Please type the perct for the item ");
				double perct= scanner_in.nextDouble();
				System.out.println("This is the retrived value"+query( attribute,  perct));
				query( attribute,  perct).printRecord();
	...
	static void menuDidsplay() 
	{
		...
		System.out.println("7. Create Array Index");
		System.out.println("8. Create BST Index ");
		System.out.println("9. Reserved ");
