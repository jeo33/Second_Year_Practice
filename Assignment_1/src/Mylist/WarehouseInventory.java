...
public class WarehouseInventory extends SinglyListDictionary<String,InventoryRecord>{
	...
	double TotalIventoryValue()
	{
		double sum=0;
		this.vList.moveToStart();
		for(int i=0;i<this.size();i++)
		{
			sum+=this.vList.getValue().getInventoryValue();
			this.vList.next();
		}
		return sum;
	}
	void printAllData()
	{
		...
		for(int i=0;i<this.size();i++)
		{
			this.vList.getValue().printRecord();
			this.vList.next();
	...
	public static void main(String args[])
	{
		...
		a.clear();
		System.out.println(a.getSize());
		a.insert(R1.getID(), R1);
		a.insert(R2.getID(), R2);
		a.insert(R3.getID(), R3);
		a.insert(R4.getID(), R4);
		a.insert(R5.getID(), R5);
		a.insert(R6.getID(), R6);
		a.insert(R7.getID(), R7);
		a.insert(R8.getID(), R8);
		a.insert(R9.getID(), R9);
		System.out.println("current size is : "+a.getSize());
		a.find("IN0008").printRecord();
		a.remove(R7.getID()).printRecord();
		System.out.println("current size is : "+a.getSize());
		 */
	...
	void command()
	{
		...
		{	
			...
			case 1:
				temp=newRecord();
				this.insert(temp.getID(), temp);
				break;
			case 2:
				System.out.println("Please type the ID for the item ");
				temp_key = scanner_in.next();
				this.remove(temp_key);
				break;
			case 3:
				this.clear();
				if(this.size()==0)System.out.println("->>>>>>>>>>>Clear all");
			...
			case 4:
				System.out.println("Please type the ID for the item ");
				temp_key = scanner_in.next();
				this.find(temp_key).printRecord();
				break;
			case 5:
				System.out.println(this.getSize());
				break;
			case 6:
				System.out.println("total value: "+TotalIventoryValue());
				break;
			}
		}while(cmd!=0);
	}
