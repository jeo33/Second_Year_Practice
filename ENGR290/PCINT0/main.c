...
void Port_Init()
{
	...
	DDRB|=(1<<DDB4)|(1<<DDB5); //set trigger pin
...
ISR( PCINT0_vect )
{
	...
	if(PINB & (1 << 0)) {TCNT1 = 0;}/* Clear Timer counter */
	else  {impulse = TCNT1;}
...
*/
    if(dist>=40){duty=255; PORTB|=(1<<DDB5);}
		else if(dist<=15){duty=0; PORTB|=(1<<DDB5);}
		else if(dist<40&dist>15)
    {duty=(255+10*(dist-40)); PORTB&=~(1<<DDB5);}
		pwm_PD3_write(duty);
